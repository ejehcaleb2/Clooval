  ...
  app.post("/api/auth/sync", async (req, res) => {
    ...
    try {
      ...
      const providerExists = async (providerId: string | undefined) => {
        ...
        const providerRes = await client.query("SELECT 1 FROM providers WHERE id = $1", [providerId]);
        return providerRes.rowCount > 0;
      };
      ...
      const requestExists = async (requestId: string | undefined) => {
        ...
        const reqRes = await client.query("SELECT 1 FROM requests WHERE id = $1", [requestId]);
        return reqRes.rowCount > 0;
      };
          ...
          if (r && r.id) {
            ...
            if (checkReq.rows.length === 0) {
              ...
              await client.query(insertReqQuery, [
                ...
                await providerExists(r.providerId) ? r.providerId : null,
              ...
              if (localStatusRank > serverStatusRank || localTime > serverTime) {
                const updateQuery = `
                  ...
                  WHERE id = $1
                `;
                const providerIdToStore = await providerExists(r.providerId)
                  ? r.providerId
                  : currentOnServer.provider_id;
      ...
      if (notifications && Array.isArray(notifications)) {
        for (const n of notifications) {
          if (n && n.id && await userExists(n.studentId)) {
            const allowedRequestId = await requestExists(n.requestId) ? n.requestId : null;
            ...
            if (checkNotif.rows.length === 0) {
              ...
              await client.query(insertNotifQuery, [
                n.id, n.studentId, n.title, n.body, n.isRead || false, allowedRequestId, n.amount, cAt
              ]);
            } else {
